...
import { Bell, AlertTriangle, Info, ShieldAlert, CheckCheck, Check } from 'lucide-react';
...
export const NotificationsView: React.FC<NotificationsViewProps> = ({ alerts, onDismiss, onMarkAllRead, onMarkAlertRead }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-200/80 shadow-[0_2px_10px_rgba(0,0,0,0.03)]">
        {(['all', 'critical', 'warning', 'info'] as const).map(f => (
          ...
          >
            {f === 'all' ? `All Alerts (${alerts.length})` : f}
          </button>
        ))}
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="text-xs font-semibold text-slate-400 hover:text-rose-600 hover:bg-rose-50 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                >
                  Dismiss
                </button>
              </div>
